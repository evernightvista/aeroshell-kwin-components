...
// KCMs are loaded into System Settings, so changing the process-wide default
// translation domain permanently would also break translations provided by
// System Settings itself.  Keep the plugin domain scoped to construction,
// where ki18n_wrap_ui-generated code resolves its strings.
...
// "#rrggbb" hex strings and SVG color names, so parse both formats here.
    ...
    : KCModule(parent, data), m_sharedMemory("kwinaero")
{
    ...
    connect(ui.kcfg_FollowPlasmaAccentColor, SIGNAL(stateChanged(int)), this, SLOT(on_kcfg_FollowPlasmaAccentColor_stateChanged(int)));
    ...
    on_kcfg_FollowPlasmaAccentColor_stateChanged(ui.kcfg_FollowPlasmaAccentColor->checkState());
...
void BlurEffectConfig::on_kcfg_FollowPlasmaAccentColor_stateChanged(int state)
{
    bool follow = (state == Qt::Checked);
    ui.showAccentColor_label->setEnabled(!follow);
    if (follow) {
        // Read Plasma accent color and apply to sliders
        ...
        if (accentColor.isValid()) {
            ...
            // Apply temporarily
            writeToMemory(ui.kcfg_AeroHue->value(), ui.kcfg_AeroSaturation->value(),
                          ui.kcfg_AeroBrightness->value(), ui.kcfg_AeroIntensity->value(),
                          ui.kcfg_EnableTransparency->isChecked(), false);
        }
    }
}
...
void BlurEffectConfig::writeToMemory(int h, int s, int v, int i, bool transparency, bool skip)
{
	...
	out << transparency;
	out << skip;
	// The effect only honors this segment for a short freshness window after
	// the write, so that colors left behind by a previous session cannot
	// override the kwinrc values at kwin startup.
...
QColor BlurEffectConfig::readPlasmaAccentColor() const
{
    ...
    auto readColor = [](const KConfigGroup &group, const char *key) {
        ...
        // "transparent" is the default value for AccentColor and is not a
        // usable accent, so continue with the color-scheme fallbacks.
